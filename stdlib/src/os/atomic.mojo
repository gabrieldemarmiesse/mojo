...
from memory import LegacyPointer, bitcast
...
struct Atomic[type: DType]:
    ...
    @always_inline
    fn _fetch_add(
        ptr: LegacyPointer[Scalar[type]], rhs: Scalar[type]
    ) -> Scalar[type]:
    ...
    fn fetch_add(inout self, rhs: Scalar[type]) -> Scalar[type]:
        ...
        Returns:
            The original value before addition.
        """
        var value_addr = LegacyPointer.address_of(self.value)
    ...
    fn fetch_sub(inout self, rhs: Scalar[type]) -> Scalar[type]:
        ...
        """
        var value_addr = LegacyPointer.address_of(self.value.value)
        return __mlir_op.`pop.atomic.rmw`[
    ...
    fn compare_exchange_weak(
        ...
        var value_integral_addr = LegacyPointer.address_of(
            self.value.value
