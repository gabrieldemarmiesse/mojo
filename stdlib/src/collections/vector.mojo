...
from memory import LegacyPointer, Reference
...
struct _VecIter[
    type: AnyRegType,
    vec_type: AnyRegType,
    deref: fn (LegacyPointer[vec_type], Int) -> type,
](Sized):
    """Iterator for any random-access container"""

    var i: Int
    var size: Int
    var vec: LegacyPointer[vec_type]
...
struct InlinedFixedVector[
    ...
    var dynamic_data: LegacyPointer[type]
    ...
    fn __init__(inout self, capacity: Int):
        ...
        self.dynamic_data = LegacyPointer[type]()
        if capacity > Self.static_size:
            self.dynamic_data = LegacyPointer[type].alloc(capacity - size)
        self.current_size = 0
    ...
    @staticmethod
    fn _deref_iter_impl(selfptr: LegacyPointer[Self], i: Int) -> type:
        return selfptr[][i]
